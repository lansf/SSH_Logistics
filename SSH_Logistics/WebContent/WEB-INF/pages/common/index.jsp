...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#btnEp").click(function(){
			//判断校验是否成功
			var v = $("#pwdForm").form("validate");//校验表单数据，成功返回true。
			if(v){//数据校验成功
				//手动校验两次输入是否一致
				var v1=$("#txtNewPass").val();
				var v2=$("#txtRePass").val();
				if(v1==v2){//两次输入一致,发送ajax请求修改密码
					var url = "${pageContext.request.contextPath}/userAction_editPassword.action";
					$.post(url,{"password":v1},function(data){//data接受的是editPassword中的flag的值
						if(data=="0"){//修改成功
							$.messager.alert("提示信息","密码修改成功!","info");
						}else{
							$.messager.alert("提示信息","密码修改失败!","warning");
						}
						$('#editPwdWindow').window('close');//关闭修改密码的窗口
					});
				}
			}
		});
	...
	// 版权信息
	function showAbout(){
		$.messager.alert("bos v1.0","管理员邮箱: xkchen@whu.edu.cn");
	}
</script>
</head>
				...
				<tr>
					<td style="width: 300px;">
						<div style="color: #999; font-size: 8pt;">
							BOS | Powered by <a href="https://github.com/xiaokangjack">xiaokangjack@github</a>
						</div>
					</td>
    ...
    <div id="editPwdWindow" class="easyui-window" title="修改密码" collapsible="false" minimizable="false" modal="true" closed="true" resizable="false"
        ...
        <div class="easyui-layout" fit="true">
            <div region="center" border="false" style="padding: 10px; background: #fff; border: 1px solid #ccc;">
                <form id="pwdForm"><!-- 该form表单用于封装数据，不需要提交到服务器，也就不需要action -->
                <table cellpadding=3>
                    <tr>
                        <td>新密码：</td>
                        <td><input id="txtNewPass" type="Password" required="true" data-options="validType:'length[6,8]'" class="txt01 easyui-validatebox" /></td>
                    </tr>
                    <tr>
                        <td>确认密码：</td>
                        <td><input id="txtRePass" type="Password"  required="true" data-options="validType:'length[6,8]'" class="txt01 easyui-validatebox" /></td>
                    </tr>
                </table>
                </form>
            </div>
